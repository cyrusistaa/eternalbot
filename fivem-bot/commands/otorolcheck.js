...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!roleId) return interaction.reply({ content: '❌ OTO_ROL_ID ayarlı değil.', ephemeral: true });

        const role = guild.roles.cache.get(roleId) || await guild.roles.fetch(roleId).catch(() => null);
        if (!role) return interaction.reply({ content: `❌ Rol bulunamadı: ${roleId}`, ephemeral: true });
        ...
        return interaction.reply({ content: `🧪 OtoRolCheck\n${lines.join('\n')}`.slice(0, 1900), ephemeral: true });
    }
};
